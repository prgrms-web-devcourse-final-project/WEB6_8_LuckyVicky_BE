...
public class RecommendationController {
    ...
    public ResponseEntity<RsData<MatchResponse>> matchProducts(
        ...
        int size = Optional.ofNullable(request.size()).orElse(50);
        ...
        log.info("페이징: page={}, size={}", page, size);
        ...
        // 4. 매칭 스코어 계산 및 정렬
        List<RecommendedItem> ranked = matcherService.scoreAndRank(
                filtered,
                request.preferences(),
                request.specPrefs()
        );
        ...
        // 5. 상위 N개 추출
        int limit = Optional.ofNullable(request.limit()).orElse(5);
        List<RecommendedItem> topN = ranked.stream().limit(limit).toList();
        
        log.info("===== 최종 추천: {}개 상품 =====", topN.size());
        
        return ResponseEntity.ok(RsData.of("200", 
                topN.size() + "개 상품을 추천했습니다", 
                new MatchResponse(topN)));
    }
}
